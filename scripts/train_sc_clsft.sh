#!/bin/bash
# Usage: bash scripts/train_sc_clsft.sh <bc_ckpt_path> [variant] [dataset] [resume_dir]
#   bc_ckpt_path: REQUIRED, path to BC pre-trained checkpoint, e.g.
#                 logs_gautschi/hmart_intent_aux-rcac/runs/2026-04-19_01-46-25/checkpoints/last.ckpt
...
#   resume_dir: optional, e.g. logs/hmart_intent_aux_clsft/runs/2026-04-25_12-00-00
#               When set, resumes CLSFT run (not BC) from that dir's last.ckpt:
...
echo "=== train_sc_clsft.sh | variant=$VARIANT | dataset=$DATASET | bc_ckpt=$BC_CKPT${RESUME_DIR:+ | resume=$RESUME_DIR} ==="
